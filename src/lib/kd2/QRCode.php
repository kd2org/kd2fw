...
class QRCode
{
	...
	protected $strinbuf = [], $eccbuf = [], $qrframe = [], $framask = [], $rlens = [], $genpoly = [];

	// Control values - $this->width is based on version, last 4 are from table.
	protected $version, $width, $neccblk1, $neccblk2, $datablkw, $eccblkwid;
	protected $ecclevel = 1;

	// set bit to indicate cell in $this->qrframe is immutable.  symmetric around diagonal
	protected function setmask($x, $y)
	{
		if ($x > $y) {
			$bt = $x;
			$x = $y;
	...
	// enter alignment pattern - black to $this->qrframe, white to mask (later black frame merged to mask)
	protected function putalign($x, $y)
	{
	...
	// Calculate and append ECC data to data block.  Block is in $this->strinbuf, indexes to buffers given.
	protected function appendrs($data, $dlen, $ecbuf, $eclen)
	{
		$i = $j = $fb = null;
		...
		for ($i = 0; $i < $dlen; $i++) {
			...
			if ($fb != 255) {     /* $fb term is non-zero */
				for ($j = 1; $j < $eclen; $j++) {
					$this->strinbuf[$ecbuf + $j - 1] = $this->strinbuf[$ecbuf + $j] ^ $this->gexp[$this->modnn($fb + $this->genpoly[$eclen - $j])];
				}
			}
			else {
	...
	protected function ismasked($x, $y)
	{
		if ($x > $y) {
			$bt = $x;
			$x = $y;
	...
	protected function applymask($m)
	{
		//var $x, $y, $r3x, $r3y;
	...
	protected $N1 = 3, $N2 = 3, $N3 = 40, $N4 = 10;

	// Using the table of the length of each run, calculate the amount of bad image 
	// - long runs or those that look like finders; called twice, once each for $X and $Y
	protected function badruns($length)
	{
		$runsbad = 0;
		for ($i = 0; $i <= $length; $i++) {
			if ($this->rlens[$i] >= 5) {
	...
	protected function genframe($instring)
	{
		//var $x, $y, $k, $t, $v, $i, $j, m;
		...
		do {
			...
			$k = $this->datablkw * ($this->neccblk1 + $this->neccblk2) + $this->neccblk2 - 3 + ($this->version <= 9);
			if ($t <= $k) {
				break;
			}
		...
		for ($i = 0; $i <= $this->eccblkwid; $i++) {
			$this->genpoly[$i] = $this->glog[$this->genpoly[$i]]; // use logs for $this->genpoly[] to save calc step
		}
		...
		for ($k = 0; $k < 8; $k++) {
			...
			$this->qrframe = $this->strinbuf; // reset for next pass
		}
		if ($t != $k) {
	 ...
	 * @param string  $text     QRCode content
	 * @param integer $ecclevel Error correction level (1 to 4)
	 */
	public function __construct($text, $ecclevel = 1)
	{
		static $max_size = [
			1 => 2953,
			2 => 2331,
		...
		$ecclevel = (int)$ecclevel;

		if (!isset($max_size[$ecclevel]))
		{
			throw new \OutOfBoundsException('Invalid ECC level, should be between 1 and 4');
		}

		if (strlen($text) > $max_size[$ecclevel])
		{
			throw new \OutOfBoundsException(sprintf('String is too long: %d bytes. QRCode maximum is %d bytes at level %d.', strlen($text), $max_size[$ecclevel], $ecclevel));
		}

		$this->ecclevel = (int) $ecclevel;
		$this->genframe($text);
	}

	/**
	 * Returns QR code as an HTML table
	 * @return string
	 */
	public function toHTML()
	{
		$out = '<table style="background: #fff; border-collapse: collapse;">';
		...
		for ($i = 0; $i < $this->width; $i++)
		{
			...
			{
				if ($this->qrframe[$j * $this->width + $i])
				{
					$out .= '<td style="background: #000; color: #000; width: 1em; height: 1em;"></td>';
				}
				else
				{
					$out .= '<td style="width: 1em; height: 1em;"></td>';
				}
			}
	...
	/**
	 * Returns QR Code as text suitable for display in command line (unicode)
	 * @return string
	 */
	public function toCLI()
	{
		$out = PHP_EOL . '  ' . str_repeat('██', $this->width+2) . PHP_EOL;
		...
		$out .= '  ' . str_repeat('██', $this->width+2) . PHP_EOL . PHP_EOL;

		return $out;
	}

	/**
	 * Returns QR code as SVG
	 * @return string
	 */
	public function toSVG()
	{
		...
		$out .= sprintf('<svg version="1.1" baseProfile="full"  width="%1$d" height="%1$d" viewBox="0 0 %1$d %1$d"', $w) . PHP_EOL;
		$out .= '  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:ev="http://www.w3.org/2001/xml-events">' . PHP_EOL;
		$out .= sprintf('<rect width="%d" height="%d" fill="#ffffff" cx="0" cy="0" />', $w, $w) . PHP_EOL;
		$out .= '<defs><rect id="p" width="8" height="8" /></defs>' . PHP_EOL;
		$out .= '<g fill="#000000">' . PHP_EOL;
		...
		for ($i = 0; $i < $this->width; $i++)
		{
			for ($j = 0; $j < $this->width; $j++)
			{
				if ($this->qrframe[$j * $this->width + $i])
				{
					$out .= sprintf('<use x="%d" y="%d" xlink:href="#p" />', $j * 8, $i * 8) . PHP_EOL;
				}
			}
		}
